...
func (rr RunnersRepository) UpdateRunner(runner *models.Runner) *models.ResponseError {
	query := `update runners set first_name = $1, last_name = $2, age = $3, country = $4 where id = $5`
...
func (rr RunnersRepository) UpdateRunnerResults(runner *models.Runner) *models.ResponseError {
	query := `update runners set personal_best = $1, season_best = $2 where id = $3`
...
func (rr RunnersRepository) GetRunnersByYear(year int) ([]*models.Runner, *models.ResponseError) {
	query := `select runners.id, runners.first_name, runners.last_name, runners.age, runners.is_active, runners.country, runners.personal_best, results.race_result from runners inner join(select runner_id, min(race_result) as race_result from results where year = $1 group by runner_id) results on runners.id = results.runner_id order by results.race_result limit 10`
